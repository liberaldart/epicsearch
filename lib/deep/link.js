...
const dualLink = (e1, e2, e1ToE2Relation, cache, isOwn, dontHandleLinking) => {
  ...
  return makeLink(e1, e1ToE2Relation, e2, cache, isOwn, dontHandleLinking)
  .then(() => makeLink(e2, e2ToE1Relation, e1, cache, isOwn, dontHandleLinking))
  /**.spread((res1, res2) => {
...
const e1UnionsAndJoins = (cache, e1, e1ToE2Relation, e2) => {
  ...
  .spread((e1Loaded, e2Loaded) => {
    ...
    return inboundGraphUpdater.execute(cache, e1Loaded, [e1ToE2Relation, e2Loaded._id], e2Loaded, unionCalculator.updateLeftNode)

    .then(() => joinCalculator.resolveForEntity(cache, null, 'index', e1Loaded, {[e1ToE2Relation]: e2Loaded}))
  })
