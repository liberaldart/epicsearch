...
const traverseThroughRightNode = (cache, pathType, leftNode, edgePathToRightNode, rightNode, callback) => {
  ...
  //debug(leftNode._type, edgePathToRightNode, rightNode._type)
  return async.each(_.values(leftNodeSchema), 
    (leftNodeFieldInfo) => {
      return traverseForFieldThroughRightNode(cache, pathType, leftNodeFieldInfo, leftNode, edgePathToRightNode, rightNode, callback)
  } )
}

const traverseForFieldThroughRightNode = (cache, pathType, leftNodeFieldInfo, leftNode, edgePathToRightNode, rightNode, callback) => {
  //Which paths to fetch depdendent field information from?
  const relationWisePathsForDependency = pathType === 'joinFrom' && _.get(leftNodeFieldInfo, [pathType, 'index']) || leftNodeFieldInfo[pathType]
  if (!relationWisePathsForDependency) {
    //debug(leftNode._type, pathType, edgePathToRightNode, rightNode._type, leftNodeFieldInfo)
    return
  }

  //Get relation wise path from edge path. Ex. sessions.speakers from sessions, 'session_id', speakers, 'speaker_id'
  const relationWisePathToRightNode = utils.extractRelationWisePath(edgePathToRightNode)
  ...
  const relationWisePathsAheadOfRightNode = 
    relationWisePathsForDependency
      .filter((path) => utils.startsWith(path, relationWisePathToRightNode)&& path.length != relationWisePathToRightNode.length)
  ...
  if (_.isEmpty(relationWisePathsAheadOfRightNode)) {
    return
  }

  debug('traverse through right node', rightNode._type, 'Left node', leftNode._type, ' for field', leftNodeFieldInfo.name, 'with dependencypath', relationWisePathsForDependency, 'and path to right node', relationWisePathToRightNode, 'path type', pathType)
  ...
  return async.each(relationWisePathsAheadOfRightNode, (relationPathAheadOfRightNode) => {

    //Get leaf entities at of that path when exploring from right node.
    const relationPathToLastParents = _.dropRight(relationPathAheadOfRightNode, 1)
    if (!relationPathToLastParents.length) {
      return Q()
    }
    return utils.getEntitiesAtRelationPath(cache, rightNode, relationPathToLastParents)
    .then((nodesFromRightGraph) => {
      ...
      if (!_.isArray(nodesFromRightGraph.entities)) {
        nodesFromRightGraph.entities = [nodesFromRightGraph.entities] 
      }
      ...
      return async.each(nodesFromRightGraph.entities, (nodeFromRightGraph) => {
        const params = {
          cache: cache,
          pathType: pathType,
          leftNode: leftNode,
          leftNodeFieldInfo: leftNodeFieldInfo, 
          rightGraphNode: nodeFromRightGraph,
          //edgePathToRightNode: edgePathToRightNode,
          relationPathToRightGraphNode: relationWisePathToRightNode.concat(_.dropRight(relationPathAheadOfRightNode, 1)),
          rightGraphNodeField: _.last(relationPathAheadOfRightNode),
          edgePathToRightNode: nodesFromRightGraph.idToEdgePathMap[nodeFromRightGraph._id]
        }
