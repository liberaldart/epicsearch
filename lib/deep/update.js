  ...
  static doAllUpdates(entity, updatedEntity, params, cache) {
    ...
    return async.each(_.keys(entitySchema), (field) => {
      ...
      if (joinIns && joinIns[field]) {
        updatePromises.push(outGraphUpdater.recalculateJoinsInDependentGraph(cache, updatedEntity, updatedField, fieldUpdate))
      } 

      if (fieldSchema.unionIn) {
        updatePromises.push(outGraphUpdater.recalculateUnionInDependentGraph(cache, entity, updatedEntity, updatedField))
      }

      if (fieldSchema.isRelationship) {
        updatePromises.push(DeepUpdater.handleRemovedLinks(cache, entity, updatedEntity, updatedField, fieldUpdate, params.isOwn))
        updatePromises.push(DeepUpdater.handleNewLinks(cache, updatedEntity, updatedField, fieldUpdate, params.isOwn))
      }

      return Q.all(updatePromises) 
      .then(() => {
        //debug('before resolving joins', JSON.stringify(updatedEntity), ' same entity in cache', JSON.stringify(cache.get(updatedEntity._id + updatedEntity._type)))
