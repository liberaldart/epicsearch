...
const DeepUpdater = class DeepUpdater {
  ...
  execute(params, cache) {
    ...
    if (!params.dontHandlLinking) {
      debug('WIth linking', params._type,  JSON.stringify(params.update))
    } else {
      debug('WIthout linking', updatedEntity._type, JSON.stringify(params.update))
    }
