 ...
 *
 */
let resolveForEntity = function(cache, langs, joinConfigPathOrConfig, entity, compulsoryJoins, edgePathToRightNode) {
  ...
  if (!joinsForEntity || _.isEmpty(edgePathToRightNode)) {
    //debug(entity._type, joinConfigPathOrConfig)
    return Q(entity)
  }

  const entityBody = _.cloneDeep(entity.fields || entity._source)
  if (_.isEmpty(entityBody)) {
    return Q(entity)
  }
  ...
  return async.each(_.keys(compulsoryJoins || joinsForEntity), (toJoinFieldName) => {
    ...
    //debug(entity._type, toJoinFieldName, entityBody[toJoinFieldName], cache.get(entity._id + entity._type))
    let toJoinEntities =
    ...
    return async.map(toJoinEntities, (toJoinEntity) => {
      ...
      if (preExistingJoinedEntityBody && !_.find(_.flatten([compulsoryJoins[toJoinFieldName]]), {_id: toJoinEntity._id})) {
      ...
      return utils.getEntity(cache, toJoinEntity._id, toJoinEntity._type) //This get will also save the docs in cache //TODO load this from cache, if not there, then es //TODO load only required fields
      .then((toJoinDoc) => {
        ...
        return toJoinDoc
      }) //TODO resolve the joins now
      .then((toJoinDoc) => {
        const fieldJoinInfo = joinsForEntity[toJoinFieldName]
        const newEdgePathToRightNode = _.drop(edgePathToRightNode, 2)

        if (!newEdgePathToRightNode || !fieldJoinInfo) {
          return toJoinDoc
        }

        const toJoinField = newEdgePathToRightNode[0]

        if (_.isObject(fieldJoinInfo[toJoinField])) {
          const toJoinDocBody = toJoinDoc._source || toJoinDoc.fields
          const fieldValues = _([toJoinDocBody[toJoinField]]).flatten().compact().map((toJoinSubEntity) => {
            return {_id: toJoinSubEntity._id}
          }).value()
          return resolveForEntity(cache, langs, fieldJoinInfo[toJoinField], toJoinDoc, {[toJoinField]: fieldValues}, newEdgePathToRightNode)
        }
    ...
    .then(() => {
      ...
      entity[toSetProp] = mergeDeep(entity._source || entity.fields, entityBody)
      return entity
    })
  })
}
