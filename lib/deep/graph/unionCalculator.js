...
module.exports.updateLeftNode = (cache, leftNode, edgePathToRightNode, rightNode) => {
  ...
  // debug('UpdateLeftNode:', leftNode._type, 'and its graph from', rightNode._type, ':', JSON.stringify(rightNode), 'in cache', JSON.stringify(cache.get(rightNode._id + rightNode._type)), 'with path', edgePathToRightNode)
  return updateLeftNodeWithRightNode(cache, leftNode, edgePathToRightNode, rightNode) //If right node is supposed to be unioned in any relations of leftNode, then do it
    .then(() => updateLeftNodeFromRightGraph(cache, leftNode, edgePathToRightNode, rightNode))
    //TODO also update left node union from which are made of a field of right node
}
...
const updateLeftNodeWithRightNode = (cache, leftNode, edgePathToRightNode, rightNode) => {
  debug('updateLeftNodeWithRightNode', leftNode._type, JSON.stringify(edgePathToRightNode))
  if (edgePathToRightNode.length === 2) { // A unionFrom relation will always be of depth > 2
    return Q()
  }
  //debug('UpdateLeftNodeWithRightNode', leftNode._type, 'and its graph from', rightNode._type, 'with path', edgePathToRightNode)
  //debug('union right node into a left node field', 'leftNode', leftNode, 'pathToRight', edgePathToRightNode, 'rightNode', rightNode)
  ...
  return async.each(_.keys(leftNodeSchema), (field) => {
    ...
    .then((entityJustBeforeRightNode) => {
      ...
      debug('recalculating union for relation', updateParams.leftNodeFieldInfo.name, 'of', leftNode._type, 'from field', updateParams.rightGraphNodeField, 'of right node', updateParams.rightGraphNode._type, '. Path to right node', updateParams.relationPathToRightGraphNode)
...
const updateLeftNodeFromRightGraph = (cache, leftNode, edgePathToRightNode, rightNode) => {
  ...
  //debug('updateLeftNodeFromRightNodeGraph', 'leftNode', leftNode, 'pathToRight', edgePathToRightNode, 'rightNode', rightNode)
