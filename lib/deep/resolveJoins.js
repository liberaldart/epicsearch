...
module.exports = function(cache, langs, joinConfigPathOrConfig, entity) {
  ...
  return async.each(_.keys(joinsForEntity), (toJoinFieldName) => {
    ...
    return async.map(_.flatten([toJoinEntities]), (toJoinEntity) => {
      ...
      .then((toJoinDocs) => {
        //debug('after deep get', toJoinEntity._type, cache.data[toJoinEntity._id+fieldSchema.to])
      //This needs a join
        toJoinDocs =
          _(toJoinDocs)
          .flatten()
        ...
        //Avoid to mess with original docs stored in cache, for they represent the actual docs. Can sanitizie/modify a copy of these docs instead
        toJoinDocs = sanitize(cache.es.config.schema, langs, toJoinDocs)
...
const sanitize = (schema, langToJoin, docs) => {
  if (!docs) {
    return
  }
...
const purgeUnwantedProperties = (docs) => {
  docs.forEach(
    (doc) => FIELDS_TO_PURGE.forEach((field) => delete doc[field])
  )
