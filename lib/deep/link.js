  ...
  return async.each(entities, (e2) => {
    ...
    .then((isAlreadyDualLinked) => {
      ...
      if (isAlreadyDualLinked) {
        ...
        //debug('starting dual linked. E1', params.e1._type, 'e2', e2._type, 'isOwn', params.isOwn, 'dontHandleLinking', params.dontHandleLinking)
        return dualLink(params.e1, e2, params.e1ToE2Relation, cache, params.isOwn, params.dontHandleLinking, params.relationPropertyIsSet)
      }
    })
  })
  .then(() => {
...
const dualLink = (e1, e2, e1ToE2Relation, cache, isOwn, dontHandleLinking, relationPropertyIsSet) => {
  ...
  return makeLink(e1, e1ToE2Relation, e2, cache, isOwn, dontHandleLinking, relationPropertyIsSet)
  .then(() => makeLink(e2, e2ToE1Relation, e1, cache, isOwn, dontHandleLinking, relationPropertyIsSet))
  /**.spread((res1, res2) => {
...
const makeLink = (e1, e1ToE2Relation, e2, cache, isOwn, dontHandleLinking, relationPropertyIsSet) => {
  if (!e1ToE2Relation) {
    return Q()
  }
  ...
  .then((isAlreadyLinked) => {
    if (!isAlreadyLinked) {
      ...
      return (relationPropertyIsSet && Q()) || setRelationPropertyInE1(cache, e1, e1ToE2Relation, e2, isOwn, dontHandleLinking)
      .then(() => {
...
const e1UnionsAndJoins = (cache, e1, e1ToE2Relation, e2) => {
  ...
  .spread((e1Loaded, e2Loaded) => {
    ...
    return traverser(cache, 'unionFrom', e1Loaded, edgePathToE2, e2Loaded, (params) => {
    ...
    .then(() => 
      ...
      traverser(cache, 'joinFrom', e1Loaded, edgePathToE2, e2Loaded, (params) =>

        joinCalculator.resolveForEntity(cache, null, 'index', params.leftNode, {[params.edgePathToRightNode[0]]: {_id: params.edgePathToRightNode[1]}})
      )
    )
	})
  .then(() => {
