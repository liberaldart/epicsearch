...
/**
 *@param {Cache} cache
 *@param {String} language to be resolved
 ...
 *
 */
module.exports = function(cache, lang, joinConfigPathOrConfig, entity) {
  const es = cache.es
  ...
  return async.each(_.keys(joins), (toJoinFieldName) => {
    ...
    let fields = fieldsToFetch.forEntity(es, fieldSchema.to, joins[toJoinFieldName], lang)
