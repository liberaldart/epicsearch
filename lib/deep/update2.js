 ...
 */
DeepUpdater.prototype.execute  = function(params, cachedEntities) {
...
DeepUpdater.prototype.updateEntityAndSiblings = function(params, cachedEntities) {
  params._type = params.type || params._type
  params._id = params._id || params.id
  const deepUpdater = this
  return utils.getEntity(cachedEntities, params._id, params._type, deepUpdater.es)
  .then((entity) => {
    ...
    return deepUpdater.doAllUpdates(entity, updatedEntity, params, cachedEntities)
  })
}

DeepUpdater.prototype.doAllUpdates = function(entity, updatedEntity, params, cachedEntities) {
  ...
  return async.each(_.keys(entitySchema), (field) => {
    ...
    if (dif && fieldSchema.copyTo) {
      ...
      updatePromises.push(
        utils.updateCopyToSiblings(updatedEntity, copyToRelations, fieldUpdate, cachedEntities, index.es))
    }
...
DeepUpdater.prototype.handleNewLinkages = function (updatedEntity, field, fieldUpdate, cachedEntities) {
...
DeepUpdater.prototype.recalculateUnionInSiblings = function(entity, updatedEntity, field, cachedEntities) {
  ...
  return async.each(unionInRelations, (relation) => {
    ...
    return async.each(toUpdateEntityIds, (relatedEId) => {
      return utils.recalculateUnionInSibling(field, entity, updatedEntity, relation, relatedEId, cachedEntities, es)
    })
  })
}

DeepUpdater.prototype.updateCopyToSiblings = function(updatedEntity, relations, fieldUpdate, cachedEntities) {
  const deepUpdater = this
  ...
  return async.each(relations, (relation) => {
    ...
    return async.each(toUpdateEntityIds, (relatedEId) => {
      return utils.updateCopyToSibling(updatedEntity, relation, relatedEId, fieldUpdate, cachedEntities, deepUpdater.es)
    })
  })
}
...
DeepUpdater.prototype.execute.doAllUpdates = DeepUpdater.prototype.doAllUpdates
