...
statement
  = get / search / async / index / link / booleanExpression / assignment / memUpdate
...
memUpdate
  = command:word val:word space+ 'at' space+ docWithPath:word {
      return {
        command: command,
...
jsonOrWord
  = val:(json / (!'if' word)) {
    console.log(_.isArray(val) && _.compact(val)[0] || val)
