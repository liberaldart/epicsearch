...
const traverseForFieldThroughRightNode = (cache, pathType, leftNodeFieldInfo, leftNode, edgePathToRightNode, rightNode, callback) => {
  ...
  const relationWisePathsAheadOfRightNode = 
    relationWisePathsForDependency
      .filter((path) => startsWith(path, relationWisePathToRightNode)&& path.length != relationWisePathToRightNode.length)
  ...
  //debug('traverse through right node', rightNode._type, 'Left node', leftNode._type, ' for field', leftNodeFieldInfo.name, 'with dependencypath', relationWisePathsForDependency, 'and path to right node', relationWisePathToRightNode)
  ...
  return async.each(relationWisePathsAheadOfRightNode, (relationPathAheadOfRightNode) => {
    ...
    return utils.getEntitiesAtRelationPath(cache, rightNode, _.dropRight(relationPathAheadOfRightNode, 1))
    .then((nodesFromRightGraph) => {
      ...
      return async.each(nodesFromRightGraph, (nodeFromRightGraph) => {
        const params = {
          cache: cache,
          pathType: pathType,
          leftNode: leftNode,
