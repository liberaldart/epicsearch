 ...
 *
 */
const setPaths = (schema, entityType, fieldName, pathType, context) => {
  let fromMePaths = _.get(schema[entityType][fieldName], [pathType, 'startingFromMe', context, entityType, fieldName, 'paths'])
  ...
  fromMePaths.forEach((path) => {
    ...
    _.dropRight(path).forEach((edge, i) => { //Last edge of path is either a simple field or relationship field. So not looping over it
      ...
      const field = _.last(path)//Either a normal field or relationship. Doesn't matter in this case

      if (isLastEdge) {
        newPathWay = [pathType, 'endingAtMe', context, entityType, field, 'paths']
      } else {
        newPathWay = [pathType, 'throughMe', context, entityType, field, 'paths']
      }


      let pathsInNextEntityType = _.get(schema[nextEntityType][field], newPathWay) || []
      if (_.isEmpty(pathsInNextEntityType)) {
        _.set(schema[nextEntityType][field], newPathWay, pathsInNextEntityType)
      }
