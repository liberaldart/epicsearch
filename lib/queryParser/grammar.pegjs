...
statement
  = get / search / async / memUpdate / index / booleanExpression / assignment
...
assignment
  = assignee:word (' is' / ' are') assignment:(!'if' word) conditionalAssignment:conditionalAssignment? {
    const res = {
      command: 'assignment',
      assignee: assignee,
...
conditionalAssignment
  = ' if' condition:booleanExpression '.'? space? 'Else ' 'is'? value:word {
    return {
      condition: condition,
