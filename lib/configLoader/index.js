...
module.exports = function(configFolderPath) {
  ...
  setJoins(config, configFolderPath + '/schema/joins.txt')
...
const setJoins = (config, configPath) => {
  ...
  try {
    ...
    joinInfo.forEach((entityWiseJoin) => {
      _.each(entityWiseJoin, (joins, entityType) => {
        joins.forEach((join) => {
          const remainingPath = [entityType, _.first(join.path), 'joins'].concat(_.drop(join.path, 1))
          _.set(config.schema, remainingPath.concat('fields'), join.fields)
          _.set(config.schema, remainingPath.concat('joins'), join.joins)
        })
      })
    })
  } catch(err) {
...
const setUnions = (config, configPath) => {
  ...
  _.keys(unions).forEach((etA) => {
    ...
    _.keys(unionsForEtA).forEach((aFieldToUnionFromB) => { //field or relationship
      ...
      //aToBPaths = _.map(aToBPaths, (aToBPath) => aToBPath.split('.'))
