...
const resolveJoins = (cache, entity, relation) => {
  ...
  const relatedEntityType = cache.es.config.schema[entity._type][relation].to

  const entitiesToJoin = _.filter(entity[relation], (toJoinEntity) => {
    toJoinEntity._type = relatedEntityType
  ...
  return async.each(entitiesToJoin, (toJoinEntity) => {
    return async.each(supportedLanguages,
