  ...
  try {
    ...
    joinInfo.forEach((entityWiseJoin) => {
      ...
      _.each(entityWiseJoin, (joinsForEntity, entityType) => {
        ...
        setJoinsForEntity(joinConfig, entityType, joinsForEntity)
...
const setInvertedJoinsForEntity = (config, invertedJoinConfig, entityType, joins) => {
  ...
  joins.forEach((join) => {
    ...
    debug(entityType, join.path, etB, pathBToA)
...
const setJoinsForEntity = (joinConfig, entityType, joins) => {
  ...
  joins.forEach((join) => {
    ...
    join.fields.forEach((fieldName) => configAtJoinPath[fieldName] = 1)
...
const reverseRelationPath = (config, etA, pathToB)  => {
  ...
  let bToAPath = _.map(pathToB, (forwardRelation) => {
    ...
    if (!forwardRelationSchema) { //TODO write a helpful error message to user to fix schema
      ...
      throw new Error('No forward relationschema for A ' + etA + '. Relation is ' + forwardRelation + '. A\'s schema is ' + config.schema[etA] + ' pathToB is ' + pathToB)
    }
