...
let resolveForEntity = function(cache, langs, joinConfigPathOrConfig, entity, compulsoryJoins, edgePathToRightNode) {
  ...
  return async.each(_.keys(compulsoryJoins || joinsForEntity), (toJoinFieldName) => {
    ...
    return async.map(toJoinEntities, (toJoinEntity) => {
      ...
      return utils.getEntity(cache, toJoinEntity._id, toJoinEntity._type) //This get will also save the docs in cache //TODO load only required fields
      .then((toJoinDoc) => {
      ...
      .then((toJoinDoc) => {
        ...
        if (_.isEmpty(newEdgePathToRightNode) || _.isEmpty(fieldJoinInfo)) {
          return toJoinDoc
        }
        ...
        if (_.isObject(fieldJoinInfo[toJoinField])) {
          ...
          const fieldValues = _.flatten([toJoinDocBody[toJoinField]])
          return resolveForEntity(cache, langs, fieldJoinInfo, toJoinDoc, {[toJoinField]: fieldValues}, newEdgePathToRightNode)
        }
    ...
    .then(() => {
      ...
      //debug(JSON.stringify(cache.get(entity._id + entity._type)), JSON.stringify(entity))
      return entity
    })
  })
}
