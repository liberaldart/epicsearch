 ...
 * @params (e2Entities or es) single entity or array of objects with _type and _id fields
 *
 */
function Index(es) {
  this.es = es
...
Index.prototype.execute = function(params, cachedEntities) {
  ...
  return async.each(entities, (e2) => {
    if (params.e1ToE2Relation) {
      return dualLink(params.e1, e2, params.e1ToE2Relation, cachedEntities, config)
    } else {
      return dualLink(e2, params.e1, params.e2ToE1Relation, cachedEntities, config)
    }
  })
  .then(() => {
...
const dualLink = (e1, e2, e1ToE2Relation, cachedEntities, config) => {
  debug(config)
  ...
  const e2ToE1Relation = e1ToE2RelationDef.revName

  return Q.all([
    makeLink(e1, e1ToE2Relation, e2, cachedEntities, config),
    makeLink(e2, e2ToE1Relation, e1, cachedEntities, config)]
  )
}

const makeLink = (e1, e1ToE2Relation, e2, cachedEntities, config) => {
  if (!e1ToE2Relation) {
    return Q()
  }
  ...
  .then((isAlreadyLinked) => {
    ...
    .then(() => {
      return updateSibling(e1, e1ToE2Relation, e2._id, cachedEntities, config)
    })
  })
}
...
const updateSibling = (e1, e2, e1ToE2Relation, cachedEntities, config) => {
  var e1Schema = config.schema[e1._type]
