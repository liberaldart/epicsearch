...
const Search = class Search {
  ...
  sanitizeAndResolve(cache, hits, params) {
    return async.each(hits, (doc) => {
      ...
      return require('./resolveJoins')(cache, params.langs, params.joins, doc)
    })
  }
  ...
  query(params) {
    ...
    const fields = fieldsToFetch.resolvePaths(this.es.config, type, params.langs, params.fields)
...
if (require.main === module) {
  ...
  es.deep.search({
      _type: 'speaker',
      langs: 'english',
      fields: ['events','primaryLanguages.name'],
      query: {}
      //context: 'web.search'
    }, cache)
    .catch((err) => {
