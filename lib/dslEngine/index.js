...
const executeDeep = (parsedInstructions, ctx, es) => {
  return async.eachSeries(parsedInstructions, (instruction) => {
    ...
    if (!mainInstruction) {
      debug('emtoy instruction', instruction, parsedInstructions)
    }
...
const executeSearch = (instruction, ctx, es) => {
  const type = val(instruction.type, ctx)
  const where = _.omit(val(instruction.where, ctx), _.isEmpty)
  const joins = val(instruction.joins, ctx)
  const def = Q.defer()
  esQuery.generate(type, where, null, {match: true}, (err, query) => {
    ...
    //debug(JSON.stringify(queryToEs))
    es.deep.search(queryToEs)
    .then((searchRes) => {
      debug(searchRes, queryToEs, 'topppppppppppppppppppp')
      if (!searchRes.hits.hits[0] && instruction.createIfNeeded) {
    ...
    .catch((err) => {
      debug('error is executeSearch', JSON.stringify(err), 'query', JSON.stringify(queryToEs))
      def.reject(err)
    })
  })
  return def.promise
...
const asyncFunctions = {
  ...
  each: (items, instruction, ctx, es) => {
    return async.each(items, (item) => {
      ...
      const newCtx = R.assocPath(['immutable', instruction.as], item, ctx)
...
const executeIndex = (instruction, ctx, es) => {
  ...
  debug(entity, instruction.entity)
