  ...
  static doAllUpdates(entity, updatedEntity, params, cache) {
    ...
    return async.each(_.keys(entitySchema), (field) => {
      ...
      if (joinIns && joinIns[field]) {
        updatePromises.push(outGraphUpdater.recalculateJoinsInDependentGraph(cache, updatedEntity, field, fieldUpdate))
      }

      const fieldSchema = entitySchema[field]
      if (fieldSchema.unionIn) {
        updatePromises.push(outGraphUpdater.recalculateUnionInDependentGraph(cache, entity, updatedEntity, field))
      }

      if (fieldSchema.isRelationship) {
        updatePromises.push(DeepUpdater.handleRemovedLinks(cache, entity, updatedEntity, field, fieldUpdate, params.isOwn))
        updatePromises.push(DeepUpdater.handleNewLinks(cache, updatedEntity, field, fieldUpdate, params.isOwn))
      }

      return Q.all(updatePromises) 
      .then(() => {
        //debug('before resolving joins', JSON.stringify(updatedEntity), ' Just finished the update', fieldUpdate)
        return resolveJoins(cache, null, 'index', updatedEntity)
      })
      /**.then(() => {
        debug('after resolving joins', JSON.stringify(updatedEntity))
      })**/
    })
  }
