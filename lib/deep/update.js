 ...
 */
  ...
  static doAllUpdates(entity, updatedEntity, params, cache) {
    ...
    return async.each(_.keys(entitySchema), (field) => {
      ...
      if (dif && fieldSchema.copyTo) {
        ...
        updatePromises.push(
          DeepUpdater.updateCopyToSiblings(updatedEntity, copyToRelations, fieldUpdate, cache))
      }
  ...
  static handleNewLinkages(updatedEntity, field, fieldUpdate, cache) {
    return async.each(['addToSet', 'push', 'set'], (updateType) => {
      ...
      var e2Type = utils.fieldType(cache.es.config, updatedEntity._type, field)
...
if (require.main === module) {
  ...
  .then((res) => {
    //debug('Updated', JSON.stringify(cache))
    return es.dsl.execute('get a 12')
    .then(debug)
    .catch((err) => {
      debig(err, err.stack, 'error in gettting')
    })
  })
  .catch(debug)
}
