 ...
 *
 */
module.exports = function(cache, langs, joinConfigPathOrConfig, entity, compulsoryJoins) {
  ...
  return async.each(_.keys(compulsoryJoins || joinsForEntity), (toJoinFieldName) => {
    ...
    .then((toJoinDocs) => {
      debug('resolved joins for ' + toJoinFieldName + ' in ' + entity._type, 'to join entities', JSON.stringify(toJoinDocs), 'entities before resolution', JSON.stringify(toJoinEntities)) 
      //This needs a join
      toJoinDocs =
        _(toJoinDocs)
        .flatten()
