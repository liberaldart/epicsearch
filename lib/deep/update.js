...
function index(esinstance) {
  // console.log("es",esinstance)
  es = esinstance
}
 ...
 */
index.prototype.executes  = (params, cachedEntities) => {
...
const doAllUpdates = (entity, updatedEntity, params, cachedEntities) => {
  var entitySchema = configs.schema[updatedEntity._type]
...
const recalculateUnionInSiblings = (entity, updatedEntity, field, cachedEntities) => {
  var entitySchema = configs.schema[updatedEntity._type]
  var fieldSchema = entitySchema[field]
...
const fieldType = (entityType, field) => {
  var fieldSchema = configs.schema[entityType][field]
