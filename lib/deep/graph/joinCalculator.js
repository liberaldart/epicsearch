  ...
  return async.each(_.keys(compulsoryJoins || joinsForEntity), (toJoinFieldName) => {
    ...
    return async.map(toJoinEntities, (toJoinEntity) => {
      ...
      /**.then((toJoinDoc) => {
        ...
        return async.each(_.keys(fieldJoinInfo), (field) => {
          if (_.isObject(fieldJoinInfo[field])) {
            const fieldValues = _([toJoinDoc[field]]).flatten().compact().map((toJoinSubEntity) => {_id: toJoinSubEntity._id}).value()
            return resolveForEntity(cache, langs, fieldJoinInfo[field], toJoinDoc, {[field]: fieldValues})
          }
        })
      })**/
    })
    ...
    .then(() => {
      ...
      //debug('joined entity', JSON.stringify(entity), JSON.stringify(cache.get(entity._id + entity._type)))
      return entity
    })
  })
}
