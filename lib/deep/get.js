...
const Get = class Get {
  ...
  execute(params, cache) {
    ...
    .then((esDoc) => {
      //Can cache this esDoc because it will be updated in memory in the subsequent flow. Let other flows work with this esDoc.
      const cached = cache.get(params._id + params._type) || cache.getEntity(params)
      if (!cached) {
    ...
    .then((joinedDoc) => {
      //Test the cache again. Maybe some other flow has added same entity to cache. 
      const cached = cache.get(params._id + params._type) || cache.getEntity(params)
      if (cached !== joinedDoc) {
        sanitize(es, cached)
...
if (require.main === module) {
  ...
  cache.es.deep.get(
    { _type: 'speaker', _id: '1', fields: [ 'primaryLanguages.fields.name' ], joins: undefined, langs: [ 'english', 'tibetan' ] }
  )
  .then(function(res) {
